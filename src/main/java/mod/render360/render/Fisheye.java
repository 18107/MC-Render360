...
public class Fisheye extends RenderMethod {
	...
	public void addButtonsToGui(List<GuiButton> buttonList, int width, int height) {
		...
		buttonList.add(new GuiButton(18110, width / 2 + 100, height / 6 - 12, (width - 10) - (width / 2 + 100), 20, (fisheyeType==0?"Stereographic":"")+(fisheyeType==1?"Equidistant":"")+(fisheyeType==2?"Equisolid":"")+(fisheyeType==3?"Thoby":"")+(fisheyeType==4?"Orthographic":"")));
	...
	public void onButtonPress(GuiButton button) {
		...
		if (button.id == 18110) {
			...
			if (fisheyeType>4)
				fisheyeType=0;
			button.displayString = (fisheyeType==0?"Stereographic":"")+(fisheyeType==1?"Equidistant":"")+(fisheyeType==2?"Equisolid":"")+(fisheyeType==3?"Thoby":"")+(fisheyeType==4?"Orthographic":"");
		}
		if (button.id == 18111) {
			fullFrame = !fullFrame;
