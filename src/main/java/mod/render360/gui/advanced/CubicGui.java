...
public class CubicGui implements Advanced {
	...
	public void initGui(List<GuiButton> buttonList, int width, int height) { //TODO change ids
		buttonList.add(new Slider(new Responder(), 18111, width / 2 - 155, height / 6 + 72, 150, 20, "Quality", 0.1f, 5f, cubic.quality, 0.1f, null));
		buttonList.add(new GuiButton(18112, width / 2 + 5, height / 6 + 72, 150, 20, "Antialiasing: " + (cubic.antialiasing == 1 ? "OFF" : cubic.antialiasing == 4 ? "LOW" : "HIGH")));
		buttonList.add(new GuiButton(18109, width / 2 - 155, height / 6 + 96, 150, 20, "Resize Gui: " + (cubic.resizeGui ? "ON" : "OFF")));
		buttonList.add(new GuiButton(18103, width / 2 + 5, height / 6 + 96, 150, 20, "Background Color: " + (cubic.skyBackground ? "Sky" : "Black")));
	}
